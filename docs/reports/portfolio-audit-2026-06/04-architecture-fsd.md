...
- **Layer inventory**: ~12 routes (+ `/es` mirror), 13 widgets, 4 features (cli-terminal,
  language-picker, splash-screen, theme-toggle), 4 entities (badge, experience, project,
  technology), shared ui/lib/config.

---

## Issues (all minor / cosmetic)

### 1. Generic `@/` alias instead of explicit layer aliases ✅ `P2-1`
...
### 3. Loose widgets outside the slice structure ✅ `P2-3`
...
| Explicit path aliases | ⚠️ ~22 `@/` instances (`P2-1`) |
| Component naming (PascalCase) | ⚠️ 3 files (`P2-2`) |
| Slice structure | ⚠️ 2 loose widgets (`P2-3`) |
