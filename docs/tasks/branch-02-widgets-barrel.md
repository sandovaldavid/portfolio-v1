...
- [ ] Audit all widget slices in `src/widgets/`
  - [ ] `about-content`
  - [ ] `about-me`
  - [ ] `badges`
  - [ ] `contact-sidebar`
...
- [ ] Export each widget using its public API name
  ```typescript
  ...
  // ... etc
  ```
- [ ] Verify no circular imports are created (widgets must not import from `src/widgets/index.ts` internally)
...
- [ ] Review `src/pages/es/index.astro`
  - [ ] If it imports >= 3 widgets, consider switching to barrel
- [ ] Review `src/pages/en/index.astro` (if exists) or other page files
- [ ] Review `src/app/layouts/Layout.astro`
  - [ ] Currently imports: Header, Footer, TechStack, RecruiterHUD, ContactSidebar
  - [ ] Candidate for partial barrel usage (group related widgets)
- [ ] Preserve single-widget imports where the barrel adds no value
...
- [ ] Verify no new lint errors (`bun run lint` or `bun run format:check`)
