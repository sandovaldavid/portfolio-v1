...
for marker in ["\t\ticon: 'projects',", "\t\ticon: 'blog',", "\t\ticon: 'about-me',"]:
    replace_once("src/widgets/header/ui/Header.astro", marker, marker + "\n\t\tprefetch: true,")

for file in ["src/widgets/header/ui/DesktopNav.astro", "src/widgets/header/ui/MobileNav.astro"]:
    replace_once(file, "\t\ticon: string;\n", "\t\ticon: string;\n\t\tprefetch?: boolean;\n")
    replace_once(
        file,
        "\t\t\t\thref={link.url}\n",
...
scripts["test:e2e:smoke"] = (
    ...
    "tests/e2e/performance.spec.ts --project=chromium"
)
scripts["performance:check"] = "node scripts/check-route-performance.mjs"
scripts["bundle:visualize"] = "ANALYZE_BUNDLE=true bun run build && bun run bundle:analyze"
...
replace_once(
    ...
    "console.log(\n"
    "\t`[bundle:analyze] Treemap: ${OUTPUT_DIR}/index.html (only when ANALYZE_BUNDLE=true)`\n"
    ");",
)
...
replace_once(
    "README.md",
    "Reports are uploaded as CI artifacts where applicable.",
    "Reports are uploaded as CI artifacts where applicable. Performance figures are build "
    "measurements governed by the reproducible methodology in "
    "[docs/PERFORMANCE.md](docs/PERFORMANCE.md), not unsupported speed claims.",
)
replace_once(
    "README.md",
...
replace_once(
    "README.es.md",
    "Los porcentajes de Vitest corresponden al [alcance unitario basado en riesgo]"
    "(docs/testing/UNIT-COVERAGE.md), no a todo el repositorio. Los artefactos generados, "
    "el conocimiento importado para agentes y las auditorías históricas están excluidos "
    "explícitamente del formateo; el código mantenido, los tests, el tooling, las "
    "configuraciones y la documentación activa permanecen cubiertos.",
    "Los porcentajes de Vitest corresponden al [alcance unitario basado en riesgo]"
    "(docs/testing/UNIT-COVERAGE.md), no a todo el repositorio. Los artefactos generados, "
    "el conocimiento importado para agentes y las auditorías históricas están excluidos "
    "explícitamente del formateo; el código mantenido, los tests, el tooling, las "
    "configuraciones y la documentación activa permanecen cubiertos. Las cifras de "
    "rendimiento son mediciones reproducibles del build según "
    "[docs/PERFORMANCE.md](docs/PERFORMANCE.md), no afirmaciones de velocidad sin evidencia.",
)
replace_once(
    "README.es.md",
    "bun run lighthouse          # Lighthouse CI\n",
...
replace_once(
    ...
    "```\n\n"
    "Configuration: `performance-budgets.json`. Methodology: "
    "[`docs/PERFORMANCE.md`](PERFORMANCE.md). The gate measures assets referenced by English "
    "and Spanish home, about, projects and blog routes.\n\n"
    "### Bundle Analysis\n",
)
replace_once(
    "docs/TESTING.md",
